...
export function signUp(formData) {
  ...
  return dispatch => new Promise(async (resolve, reject) => {
    ...
    if (!password_confirmation) return reject({ message: ErrorMessages.missingPassword });
    if (password !== password_confirmation) return reject({ message: ErrorMessages.passwordsDontMatch });

    await statusMessage(dispatch, 'loading', true);
    const myUser = new UserAPI();
    myUser.initialize(null);
    // Go to Firebase
    return myUser.userSignup(formData).then((res) => {
...
export function login(formData) {
  ...
  return dispatch => new Promise(async (resolve, reject) => {
    ...
    myUser.initialize(null);
    // Go to Laravel API
    return myUser.userLogin(formData)
      .then((res) => {
        return resolve(dispatch({
          type: 'USER_LOGIN',
          data: res.user,
